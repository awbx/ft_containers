...
template <class InputIterator>
typename ft::iterator_traits<InputIterator>::difference_type _distance(InputIterator first, InputIterator last, std::input_iterator_tag) {
    typename ft::iterator_traits<InputIterator>::difference_type diff(0);
    for (; first != last; ++first) ++diff;
    return diff;
}
...
typename ft::iterator_traits<_RandIter>::difference_type _distance(_RandIter first, _RandIter last, ft::random_access_iterator_tag) {
    return last - first;
}

template <class _RandIter>
typename ft::iterator_traits<_RandIter>::difference_type _distance(_RandIter first, _RandIter last, std::random_access_iterator_tag) {
    return last - first;
}
